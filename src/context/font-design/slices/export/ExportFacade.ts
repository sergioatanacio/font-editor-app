import { exportFsmInitialState, exportFsmTransition, type ExportFsmState } from "./exportTtfFsm";
import type { AppError } from "../../shared/errors/AppError";
import { ExportTypefaceToTtfUseCase } from "./ExportTypefaceToTtfUseCase";
import { ValidateTypefaceForExportUseCase } from "./ValidateTypefaceForExportUseCase";
...
export class ExportFacade {
  ...
  constructor(
    ...
    private readonly eventBus: DomainEventBus = ExportFacade.noopEventBus,
  ) {}
  ...
  async validateReadiness(projectId: string): Promise<ExportReadinessReport> {
    ...
    try {
      await this.eventBus.publish({
        name: DomainEventNames.ExportReadinessEvaluated,
      ...
      console.error("[EXPORT_TRACE][EVENT_BUS] validate:publish:error", {
  ...
  async exportTtf(input: { projectId: string; filename: string }) {
    ...
    try {
      await this.eventBus.publish({
        name: DomainEventNames.TypefaceExported,
        occurredAt: new Date().toISOString(),
        version: 1,
        ...
        payload: {
          ...
          byteLength: result.value.byteLength,
          warningCodes: result.value.warnings.map((x) => x.code),
        },
      });
    } catch (error) {
      console.error("[EXPORT_TRACE][EVENT_BUS] export:publish:error", {
        code: "EVENT_BUS_PUBLISH_ERROR",
        cause: error instanceof Error ? error.message : "unknown",
      });
    }
